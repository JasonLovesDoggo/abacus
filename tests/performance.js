...
export function setup() {
  console.log(
    `Running performance tests against ${BASE_URL} at ${new Date().toISOString()}`,
  );
...
export default function (data) {
  ...
  check(createResponse, {
    "counter created or already exists": (r) =>
  ...
  if (adminKey || __ITER % 5 === 0) {
    ...
    if (adminKey) {
      ...
      http.post(`${BASE_URL}/set/${fullKey}?value=10`, null, { headers });
    }
  }
}
